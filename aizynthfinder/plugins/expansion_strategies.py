...
class ModelZooExpansionStrategy(ExpansionStrategy):
    ...
    def _update_cache(self, molecules: Sequence[TreeMolecule]) -> None:
        ...
        # Generate results per example in the list
        for smiles in smiles_list:
            reactants, priors = self.model_zoo.model_call([smiles])
            reactants_list.append(np.array(reactants[0]))
            priors_list.append(np.array(priors[0]))

        #print("Model Call Results")
        #print(reactants_list, priors_list)
        ...
        for reactants, priors, inchi in zip(reactants_list, priors_list, pred_inchis):
            #print(reactants, priors, inchi)
