...
class TreeAnalysis:
    ...
    def _collect_top_items(
        ...
        # we need to rewrite the function to be able to return all (or the top-n) routes

        seen_hashes = set()
        best_items: List[Any] = []
        best_scores = []
        one_route_found = False

        # if we return all the values, we return all solved routes
        if selection.return_all:
            print("RETURN EVERYTHING")
            for score, item, actions in zip(scores, items, reactions):
                # we need to add at least one route or the processing will break... even an unsolved one
                if not one_route_found:
                    one_route_found = True
                # if the tree is not solved continue
                elif not item.is_solved:
                    continue

                # remove duplicate routes. we only take the first. hash of the reactions is created
                ...
                if len(best_items) >= selection.nmax:
                    # we return only the top-n routes
                    print(f"We return only the top- {selection.nmax} routes. If you want to return even more, increase this value...")
