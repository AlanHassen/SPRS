...
class RouteCollection:
    ...
    def cluster(
        ...
        **kwargs: Any,
        ...
        """
        max_clusters = 5
        print(f"Maximum number of cluster: {max_clusters}")
    ...
    def distance_matrix(
        ...
        max_clustered_routes = 100
        print(f"We cluster at maximum {max_clustered_routes} top-routes")
        top_n_routes = self.dicts[:max_clustered_routes]
        print(f"Number of routes: {len(top_n_routes)}")
        distances = calculator(top_n_routes)

        # old: cluster everything
