...
class MctsNode:
    ...
    def expand(self) -> None:
        ...
        (
            temp_children_actions,
            temp_children_priors,
        ) = self._expansion_policy(self.state.expandable_mols, cache_molecules)

        # If we get no actions, we don't need to cluster
        if len(temp_children_actions) != 0:
            ...
            for reaction in temp_children_actions:
                try:
            ...
            assert len(disconnection_importance) == len(temp_children_actions)
            assert len(disconnection_importance) == len(temp_children_priors)

            # filter out the reactions that are not important
            reactions_to_expand_clustered = [rxn for rxn, imp in zip(temp_children_actions, disconnection_importance) if imp == 1]
            reaction_priors_clustered = [cost for cost, imp in zip(temp_children_priors, disconnection_importance) if imp == 1]
            ...
            if len(reactions_to_expand_clustered) != n_clusters_:
                ...
                f"Original reactions to expand: {len(temp_children_actions)}"
                )
                reactions_to_expand_clustered = temp_children_actions
                reaction_priors_clustered = temp_children_priors
            ...
            assert len(reaction_priors_clustered) == n_clusters_

            self.tree.profiling["clustered_tree_width"].append(n_clusters_)
            self.tree.profiling["tree_width"].append(len(disconnection_importance))
            
            verbose = False
            if verbose:
                print(f"Number of clusters: {n_clusters_}")
                print(f"Number of reactions: {len(temp_children_actions)}")
                ...
                print(f"Reactions: {[reaction.reaction_smiles() for reaction in temp_children_actions]}")
                print(f"Clustered reactions: {[reaction.reaction_smiles() for reaction in reactions_to_expand_clustered]}")

            # replace the children actions and priors with the new ones
            self._children_actions = reactions_to_expand_clustered
            self._children_priors = reaction_priors_clustered
        else:
            print("No actions to expand, skipping clustering")
            self._children_actions = temp_children_actions
            self._children_priors = temp_children_priors

        # default mcts implementaiton
